...
public class Order {
	...
	public String toString() {
		...
		System.out.println(String.format("%-10s|%-10s|%-20s|%-10s|%-10s|%-20s", "STT", "Ma SP", "Mo ta", "Don gia",
				"So luong", "Thanh tien"));

		for (int i = 0; i < count; i++) {
			System.out.println(String.format("%-10s|%-10s|%-20s|%-10s|%-10s|%-20s", i + 1,
					lineItems[i].getProduct().getProductID(), lineItems[i].getProduct().getDescription(),
					nf.format(lineItems[i].getProduct().getPrice()), lineItems[i].getQuantity(),
					nf.format(lineItems[i].calcTotalPrice())));
		}
